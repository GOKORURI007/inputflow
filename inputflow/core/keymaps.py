...
if platform.system() == "Windows" or platform.system() == "Darwin":
    ...
    HID_TO_VK_BTN = {v: k for k, v in VK_TO_HID_BTN.items()}
...
NAME_TO_HID_BTN = {
    ...
    "BTN_RIGHT": 0x02,
    "BTN_MIDDLE": 0x03,
    "BTN_SIDE": 0x04,  # X1 / Back
    "BTN_EXTRA": 0x05,  # X2 / Forward
}

HID_TO_NAME_BTN = {v: k for k, v in NAME_TO_HID_BTN.items()}
...
NAME_TO_HID_KEY = {
    ...
    "KEY_LEFTSHIFT": 0xE1,
    "KEY_LEFTALT": 0xE2,
    "KEY_LEFTMETA": 0xE3,  # GUI / Win / Super
    "KEY_RIGHTCTRL": 0xE4,
    "KEY_RIGHTSHIFT": 0xE5,
    "KEY_RIGHTALT": 0xE6,
    "KEY_RIGHTMETA": 0xE7,
    ...
    # --- Alphabet (KEY_A=30 -> 0x04) ---
    # Linux evdev 的字母分布不像 VK 那样连续，建议显式映射或使用 e.KEY_A 引用
    **{f"KEY_{c}": (ord(c) - ord("A") + 0x04) for c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ"},
    # --- Digits (KEY_1=2 -> 0x1E) ---
...
def vk_to_hid_key(vk_code):
    return VK_TO_HID_KEY.get(vk_code, 0x00)


def hid_to_vk_key(hid_usage_id):
    return HID_TO_VK_KEY.get(hid_usage_id, 0x00)


def vk_to_hid_btn(vk_code):
    return VK_TO_HID_BTN.get(vk_code, 0x00)


def hid_to_vk_btn(hid_usage_id):
    return HID_TO_VK_BTN.get(hid_usage_id, 0x00)


def ecode_to_hid_key(code):
    return ECODE_TO_HID_KEY.get(code, 0x00)


def hid_to_ecode_key(hid_id):
    return HID_TO_ECODE_KEY.get(hid_id, e.KEY_RESERVED)


def ecode_to_hid_btn(code):
    return ECODE_TO_HID_BTN.get(code, 0x00)


def hid_to_ecode_btn(hid_id):
    return HID_TO_ECODE_BTN.get(hid_id, e.KEY_RESERVED)


def name_to_hid_key(name):
    return NAME_TO_HID_KEY.get(name, 0x00)


def hid_to_name_key(hid_id):
    return HID_TO_NAME_KEY.get(hid_id, "UNKNOWN")


def name_to_hid_btn(name):
    return NAME_TO_HID_BTN.get(name, 0x00)


def hid_to_name_btn(hid_id):
    return HID_TO_NAME_BTN.get(hid_id, "UNKNOWN")
