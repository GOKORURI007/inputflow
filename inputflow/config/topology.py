...
class TopologyManager:
    ...
    def load_topology(self, topology_entries: List[TopologyEntry], current_ip: str) -> None:
        """Load topology configuration."""
        self._topology = topology_entries
        self._current_ip = current_ip
        logger.info(f"Loaded topology with {len(topology_entries)} entries for IP {current_ip}")
    ...
    def should_switch_screen(self, x: int, y: int) -> Optional[str]:
        ...
        for direction in Direction:
            if self.is_at_edge(x, y, direction):
                ...
                if target:
                    logger.info(f"Screen switch triggered: {direction.value} to {target}")
                    return target
        
        return None
    
    def get_edge_coordinates_for_direction(self, direction: Direction) -> Tuple[int, int]:
    ...
    def validate_topology(self) -> bool:
        ...
        if not has_direction:
            logger.warning(f"No directional relationships configured for IP {self._current_ip}")
